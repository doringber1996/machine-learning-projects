...
def Batch_Gradient_Descent(X,Y,a,b,alpha,iteration=1000):
    ...
    for epoch in range(iteration):
        yhat = a*X +b
        grad_a = (-2/N)*np.sum(X*(Y-yhat))
        grad_b = (-2/N)*np.sum(Y-yhat)
        a = a- alpha*grad_a
        b = b- alpha*grad_b
...
def Mini_Batch_Gradient_Descent(X,Y,a,b,alpha,iteration=1000,BatchSize=32):
    ...
    for epoch in range(iteration):
        ...
        a = a - alpha*grad_a
        b = b - alpha*grad_b
        loss = (np.sum(((yi-yhat)**2),axis=0))/N
        A.append(a)
        B.append(b)
        LOSS.append(loss)
...
# A,B,LOSS=Batch_Gradient_Descent(X,Y,a,b,0.0001,1000)
...
# A,B,LOSS=Batch_Gradient_Descent(X,Y,a,b,L,1000)
...
# A,B,LOSS=Stochastic_Gradient_Descent(X,Y,a,b,l,1000)
...
# A,B,LOSS=Mini_Batch_Gradient_Descent(X,Y,a,b,l,1000,Batch_Size)
